...
def write_tilemap_bin(tilemap_list: list[TileInfo], path: str):
    with open(path, "wb") as f:
        for tile in tilemap_list:
            tile_data = (tile.index & 0x3FF) | ((1 if tile.hflip else 0) << 10) | ((1 if tile.vflip else 0) << 11)
...
def handle_create(parser: argparse.ArgumentParser, args):
    try:
        ...
        if args.output_tilemap:
            write_tilemap_bin(tilemap_list, args.output_tilemap)
